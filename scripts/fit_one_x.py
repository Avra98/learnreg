...
train, val, A = src.make_data(n=100, m=100, sigma=sigma, train_size=1, val_size=1)
...
_, ax = src.solve_and_plot(A, train, W, ax=axes[0])
ax.plot(W@train.x, '.')
ax.set_title('training signal')


# try this W on another problem
_, ax = src.solve_and_plot(A, val, W, ax=axes[1])
